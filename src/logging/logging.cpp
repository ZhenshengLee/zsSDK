...
// environment variables defined in zsatypes.h
//

#define ZSA_LOGGER "zsa_logger"
...
static const char ZSA_ENABLE_LOG_TO_STDOUT[] = "ZSA_ENABLE_LOG_TO_STDOUT";
static const char ZSA_LOG_LEVEL[] = "ZSA_LOG_LEVEL";
static const char ZSA_LOG_FILE_NAME[] = "zsa.log";
static size_t ZSA_LOG_FILE_50MB_MAX_SIZE = (1048576 * 50);
...
typedef struct
{
    ...
    zsa_logging_message_cb_t *user_callback;
    void *user_callback_context;
    zsa_log_level_t user_log_level;

    // Logger data used for forwarding debug message to stdout or a dedicated zsa file.
    std::shared_ptr<spdlog::logger> env_logger;
    bool env_logger_is_file_based;
    zsa_log_level_t env_log_level;
...
// singleton global
ZSA_DECLARE_GLOBAL(logger_global_context_t, logger_init_once);

// To deal with destroying the singleton we create a static class and use the destructor to tear down the object.
// For now this is separate from ZSA_DECLARE_GLOBAL as it is C-based and can be reused in the SDK.
                                              ...
                                              zsa_log_level_t min_level)
{
    ...
    if (message_cb)
    {
        RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, min_level < ZSA_LOG_LEVEL_CRITICAL);
        RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, min_level > ZSA_LOG_LEVEL_OFF);
    }
...
static void logger_init_once(logger_global_context_t *global)
{
    ...
    global->env_log_level = ZSA_LOG_LEVEL_OFF;
    global->user_log_level = ZSA_LOG_LEVEL_OFF;
    ...
    enable_file_logging = environment_get_variable(ZSA_ENV_VAR_LOG_TO_A_FILE);
    enable_stdout_logging = environment_get_variable(ZSA_ENABLE_LOG_TO_STDOUT);
    logging_level = environment_get_variable(ZSA_LOG_LEVEL);

    if (enable_file_logging && enable_file_logging[0] != '\0')
    {
        // enable_file_logging is valid see if its a file path
        const char *log_file = ZSA_LOG_FILE_NAME;
        ...
        if (log_file)
        {
            try
            {
                ...
                global->env_logger =
                    spdlog::rotating_logger_mt(ZSA_LOGGER, log_file, ZSA_LOG_FILE_50MB_MAX_SIZE, LOG_FILE_MAX_FILES);
    ...
    if (ZSA_SUCCEEDED(result) && (global->env_logger == nullptr))
    {
        ...
        if (enable_stdout_logger)
        {
            ...
            global->env_logger = spdlog::stdout_logger_mt(ZSA_LOGGER);
        }
    }

    if (ZSA_SUCCEEDED(result) && (global->env_logger))
    {
        global->env_log_level = ZSA_LOG_LEVEL_ERROR;
        ...
        if (logging_level && logging_level[0] != '\0')
        {
            ...
            {
                // capture a severity of error or higher
                global->env_log_level = ZSA_LOG_LEVEL_CRITICAL;
            }
        }
...
#pragma GCC diagnostic pop
    ...
    if ((level <= g_context->user_log_level) && (g_context->user_log_level != ZSA_LOG_LEVEL_OFF))
    {
