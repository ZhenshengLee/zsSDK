...
#define COLOR_CAMERA_PID 0x097d // ZSA
...
zsa_result_t UVCCameraReader::Init(const char *serialNumber)
{
    if (IsInitialized())
    {
    ...
    return ZSA_RESULT_SUCCEEDED;
}

zsa_result_t UVCCameraReader::Start(const uint32_t width,
...
{
    RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, pCallback == NULL);
    RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, pCallbackContext == NULL);
    ...
    case ZSA_IMAGE_FORMAT_COLOR_BGRA32:
        m_output_image_format = imageFormat;
        m_input_image_format = ZSA_IMAGE_FORMAT_COLOR_MJPG;
...
zsa_result_t UVCCameraReader::GetCameraControlCapabilities(const zsa_color_control_command_t command,
                                                           color_control_cap_t *capabilities)
{
    ...
    case ZSA_COLOR_CONTROL_AUTO_EXPOSURE_PRIORITY:
    {
        ...
        capabilities->defaultMode = ZSA_COLOR_CONTROL_MODE_MANUAL;
        capabilities->valid = true;
        LOG_WARNING("ZSA_COLOR_CONTROL_AUTO_EXPOSURE_PRIORITY is deprecated and does nothing.", 0);
    }
    break;
    default:
...
zsa_result_t UVCCameraReader::GetCameraControl(const zsa_color_control_command_t command,
                                               zsa_color_control_mode_t *mode,
                                               int32_t *pValue)
{
    RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, mode == NULL);
    RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, pValue == NULL);
    if (!IsInitialized())
    {
        LOG_ERROR("Camera reader is not initialized", 0);
        return ZSA_RESULT_FAILED;
...
zsa_result_t UVCCameraReader::SetCameraControl(const zsa_color_control_command_t command,
                                               ...
                                               int32_t newValue)
{
    ...
    {
    case ZSA_COLOR_CONTROL_EXPOSURE_TIME_ABSOLUTE:
        if (mode == ZSA_COLOR_CONTROL_MODE_MANUAL)
        {
        ...
        else if (mode == ZSA_COLOR_CONTROL_MODE_AUTO)
        {
...
void UVCCameraReader::Callback(uvc_frame_t *frame)
{
    ...
    if (m_streaming && frame)
    {
        ...
        if (m_input_image_format == ZSA_IMAGE_FORMAT_COLOR_MJPG &&
            m_output_image_format == ZSA_IMAGE_FORMAT_COLOR_BGRA32)
        {
        ...
        // Allocate ZSA Color buffer
        buffer = allocator_alloc(ALLOCATION_SOURCE_COLOR, buffer_size);
        zsa_result_t result = ZSA_RESULT_FROM_BOOL(buffer != NULL);
        ...
        if (ZSA_SUCCEEDED(result))
        {
            ...
            image_set_device_timestamp_usec(image, ZSA_90K_HZ_TICK_TO_USEC(framePTS));
...
{
    RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, m_width_pixels * m_height_pixels * 4 > out_size);
