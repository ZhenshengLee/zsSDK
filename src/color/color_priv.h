...
typedef struct _color_control_cap_t
{
    ...
    zsa_color_control_mode_t defaultMode;
    bool supportAuto;
    bool valid;
} color_control_cap_t;
...
typedef struct _exposure_mapping
{
    ...
    int exposure_mapped_50Hz_usec; // Exposure when ZSA_COLOR_CONTROL_POWERLINE_FREQUENCY == 50Hz
    int exposure_mapped_60Hz_usec; // Exposure when ZSA_COLOR_CONTROL_POWERLINE_FREQUENCY == 60Hz
 ...
 */
typedef void(color_cb_stream_t)(zsa_result_t result, zsa_capture_t capture_handle, void *context);
