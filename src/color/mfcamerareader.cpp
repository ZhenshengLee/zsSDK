...
{
    const char *copy_to_new_buffer = environment_get_variable("ZSA_MF_COPY_TO_NEW_BUFFER");
...
zsa_result_t CMFCameraReader::Start(const UINT32 width,
                                    const UINT32 height,
                                    const float fps,
...
{
    RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, pCallback == nullptr);
    RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, pCallbackContext == nullptr);
    HRESULT hr = S_OK;
        ...
        while (!typeFound)
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (FAILED(hr = MFGetAttributeSize(spMediaType.Get(), MF_MT_FRAME_SIZE, &tempWidth, &tempHeight)))
                {
                    ...
                    return zsaResultFromHRESULT(hr);
                }
...
zsa_result_t CMFCameraReader::GetCameraControlCapabilities(const zsa_color_control_command_t command,
                                                           color_control_cap_t *capabilities)
{
    ...
    ULONG stepValue = 0;
    LONG defaultValue = 0;
    zsa_color_control_mode_t defaultMode = ZSA_COLOR_CONTROL_MODE_MANUAL;
    RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, capabilities == NULL);

    switch (command)
    {
    case ZSA_COLOR_CONTROL_EXPOSURE_TIME_ABSOLUTE:
    {
        ...
        defaultMode = ZSA_COLOR_CONTROL_MODE_AUTO;

        // Windows KsProperty uses exposure time value as log base 2 seconds, which is not linear.
        // But ZSA color control API allow to use micro sec unit exposure time.
        // Set step value to 1.
        stepValue = 1;
    }
    break;
    case ZSA_COLOR_CONTROL_BRIGHTNESS:
    {
    ...
    break;
    case ZSA_COLOR_CONTROL_CONTRAST:
    {
        hr = GetCameraControlCap(PROPSETID_VIDCAP_VIDEOPROCAMP,
    ...
    break;
    case ZSA_COLOR_CONTROL_SATURATION:
    {
        hr = GetCameraControlCap(PROPSETID_VIDCAP_VIDEOPROCAMP,
    ...
    break;
    case ZSA_COLOR_CONTROL_SHARPNESS:
    {
        hr = GetCameraControlCap(PROPSETID_VIDCAP_VIDEOPROCAMP,
    ...
    break;
    case ZSA_COLOR_CONTROL_WHITEBALANCE:
    {
        hr = GetCameraControlCap(PROPSETID_VIDCAP_VIDEOPROCAMP,
    ...
    break;
    case ZSA_COLOR_CONTROL_BACKLIGHT_COMPENSATION:
    {
        hr = GetCameraControlCap(PROPSETID_VIDCAP_VIDEOPROCAMP,
    ...
    break;
    case ZSA_COLOR_CONTROL_POWERLINE_FREQUENCY:
    {
        hr = GetCameraControlCap(PROPSETID_VIDCAP_VIDEOPROCAMP,
    ...
    break;
    case ZSA_COLOR_CONTROL_AUTO_EXPOSURE_PRIORITY:
    {
        supportAuto = false;
        minValue = 0;
        maxValue = 0;
        stepValue = 0;
        defaultValue = 0;
        LOG_WARNING("ZSA_COLOR_CONTROL_AUTO_EXPOSURE_PRIORITY is deprecated and does nothing.");
    }
    break;
    default:
        return ZSA_RESULT_FAILED;
...
zsa_result_t CMFCameraReader::GetCameraControl(const zsa_color_control_command_t command,
                                               ...
                                               int32_t *pValue)
{
    ...
    // clear return
    *mode = ZSA_COLOR_CONTROL_MODE_MANUAL;
    *pValue = 0;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
            *mode = ZSA_COLOR_CONTROL_MODE_AUTO;
        }
...
zsa_result_t CMFCameraReader::SetCameraControl(const zsa_color_control_command_t command,
                                               ...
                                               int32_t newValue)
{
    ...
    if (mode == ZSA_COLOR_CONTROL_MODE_AUTO)
    {
        if (command == ZSA_COLOR_CONTROL_EXPOSURE_TIME_ABSOLUTE || command == ZSA_COLOR_CONTROL_WHITEBALANCE)
        {
            flags = KSPROPERTY_CAMERACONTROL_FLAGS_AUTO;
        }
        else
        {
            LOG_ERROR("ZSA_COLOR_CONTROL_MODE_AUTO is not supported for %d color control", command);
            return ZSA_RESULT_FAILED;
        }
    }
...
zsa_result_t CMFCameraReader::CreateImage(CFrameContext *pFrameContext, zsa_image_t *image)
{
...
zsa_result_t CMFCameraReader::CreateImageCopy(CFrameContext *pFrameContext, zsa_image_t *image)
{
    ...
    result = ZSA_RESULT_FROM_BOOL(buffer != NULL);
                                           ...
                                           IMFSample *pSample)
{
    ...
    if (SUCCEEDED(hrStatus))
    {
        ...
        if (m_started && !m_flushing)
        {
            if (pSample && m_pCallback && m_pCallbackContext)
            {
                ...
                result = ZSA_RESULT_FROM_BOOL(pFrameContext != NULL);

                if (ZSA_SUCCEEDED(result) && pFrameContext->GetPTSTime() == 0)
                {
                    dropped = true;
                ...
                if (ZSA_SUCCEEDED(result))
                {
                    image_set_device_timestamp_usec(image, ZSA_90K_HZ_TICK_TO_USEC(pFrameContext->GetPTSTime()));
    ...
    else
    {
        ...
        m_pCallback(ZSA_RESULT_FAILED, NULL, m_pCallbackContext);
    }
