...
#define ZSA_RESULT_FROM_LIBUSB(_call_) TraceLibUsbError((_call_), #_call_, __FILE__, __LINE__, __func__)
...
// scale the libusb debug verbosity to match zsa
static zsa_result_t usb_cmd_set_libusb_debug_verbosity(usbcmd_context_t *usbcmd)
{
    zsa_result_t result = ZSA_RESULT_SUCCEEDED;
    ...
    result = ZSA_RESULT_FROM_LIBUSB(libusb_set_option(libusb_ctx, LIBUSB_OPTION_LOG_LEVEL, level));
    // #else
...
{
    // #if (LIBUSB_API_VERSION >= 0x01000106)
    ZSA_RESULT_FROM_LIBUSB(libusb_set_option(context, LIBUSB_OPTION_LOG_LEVEL, LIBUSB_LOG_LEVEL_NONE));
    // #else
...
{
    // #if (LIBUSB_API_VERSION >= 0x01000106)
    ZSA_RESULT_FROM_LIBUSB(libusb_set_option(context, LIBUSB_OPTION_LOG_LEVEL, verbosity));
    // #else
    //     libusb_set_debug(context, 3); // set verbosity level to 3, as suggested in the documentation
    // #endif
}

static zsa_result_t populate_container_id(usbcmd_context_t *usbcmd)
{
    ...
    result = ZSA_RESULT_FROM_LIBUSB(libusb_get_bos_descriptor(usbcmd->libusb, &bos_desc));
...
static zsa_result_t populate_serialnumber(usbcmd_context_t *usbcmd, struct libusb_device_descriptor *desc)
{
    zsa_result_t result;

    result = ZSA_RESULT_FROM_BOOL(desc->iSerialNumber);

    if (ZSA_SUCCEEDED(result))
    {
        result = ZSA_RESULT_FROM_LIBUSB(libusb_get_string_descriptor_ascii(usbcmd->libusb,
                                                                           desc->iSerialNumber,
...
static zsa_result_t find_libusb_device(uint32_t device_index,
                                       ...
                                       usbcmd_context_t *usbcmd)
{
    ...
    // Initialize library
    result = ZSA_RESULT_FROM_LIBUSB(libusb_init(&usbcmd->libusb_context));
    ...
    if (ZSA_SUCCEEDED(result))
    {
        ...
        result = ZSA_RESULT_FROM_BOOL((count = libusb_get_device_list(usbcmd->libusb_context, &dev_list)) < INT32_MAX);
        libusb_logging_restore(usbcmd->libusb_context, usbcmd->libusb_verbosity);
    }

    if (ZSA_SUCCEEDED(result))
    {
        result = ZSA_RESULT_FROM_BOOL(count != 0);
    }
    ...
    if (ZSA_SUCCEEDED(result))
    {
        ...
        {
            found = false;
            result = ZSA_RESULT_FROM_LIBUSB(libusb_get_device_descriptor(dev_list[loop], desc));

            if (ZSA_SUCCEEDED(result))
            {
                // Check if this is our device and correlates to the index number based on discovery order
                if ((desc->idVendor != ZSA_MSFT_VID) || (desc->idProduct != usbcmd->pid) ||
...
zsa_result_t usb_cmd_create(usb_command_device_type_t device_type,
                            uint32_t device_index,
                            const guid_t *container_id,
                            usbcmd_t *usbcmd_handle)
{
    RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, device_type >= USB_DEVICE_TYPE_COUNT);
    zsa_result_t result = ZSA_RESULT_FAILED;
    libusb_device **dev_list = NULL; // pointer to pointer of device, used to retrieve a list of devices
    struct libusb_device_descriptor desc;
    ssize_t count = 0; // holding number of devices in list
    ...
    result = ZSA_RESULT_FROM_BOOL((usbcmd = usbcmd_t_create(usbcmd_handle)) != NULL);

    if (ZSA_SUCCEEDED(result))
    {
        usbcmd->stream_going = false;
        result = ZSA_RESULT_FROM_BOOL((usbcmd->lock = Lock_Init()) != NULL);
    }
    ...
    if (ZSA_SUCCEEDED(result))
    {
        ...
        result = ZSA_RESULT_FROM_LIBUSB(libusb_get_configuration(usbcmd->libusb, &activeConfig));
    }

    if (ZSA_SUCCEEDED(result) && (activeConfig != USB_CMD_DEFAULT_CONFIG))
    {
        result = ZSA_RESULT_FROM_LIBUSB(libusb_set_configuration(usbcmd->libusb, USB_CMD_DEFAULT_CONFIG));
    }
    ...
    if (ZSA_SUCCEEDED(result))
    {
        ...
        {
            result = ZSA_RESULT_FROM_LIBUSB(libusb_detach_kernel_driver(usbcmd->libusb, usbcmd->interface));
        }
    }
    ...
    {
        // claim interface
        result = ZSA_RESULT_FROM_LIBUSB(libusb_claim_interface(usbcmd->libusb, usbcmd->interface));
    }
...
void usb_cmd_destroy(usbcmd_t usbcmd_handle)
{
    ...
    {
        // Release interface(s)
        (void)ZSA_RESULT_FROM_LIBUSB(libusb_release_interface(usbcmd->libusb, usbcmd->interface));
    }
...
zsa_buffer_result_t usb_cmd_get_serial_number(usbcmd_t usbcmd_handle, char *serial_number, size_t *serial_number_size)
{
    RETURN_VALUE_IF_HANDLE_INVALID(ZSA_BUFFER_RESULT_FAILED, usbcmd_t, usbcmd_handle);
    ...
    if (ZSA_FAILED(ZSA_RESULT_FROM_BOOL((usbcmd = usbcmd_t_get_context(usbcmd_handle)) != NULL)))
    {
        result_b = ZSA_BUFFER_RESULT_FAILED;
    }

    size_t required_length = strlen((const char *)usbcmd->serial_number) + 1;

    if ((result_b == ZSA_BUFFER_RESULT_SUCCEEDED) && (required_length > *serial_number_size))
    {
        *serial_number_size = required_length;
        result_b = ZSA_BUFFER_RESULT_TOO_SMALL;
    }

    if ((result_b == ZSA_BUFFER_RESULT_SUCCEEDED) && (serial_number == NULL))
    {
        LOG_ERROR("serial_number buffer cannot be NULL", 0);
        result_b = ZSA_BUFFER_RESULT_FAILED;
    }

    if (result_b == ZSA_BUFFER_RESULT_SUCCEEDED)
    {
        *serial_number_size = required_length;
        memset(serial_number, 0, *serial_number_size);
        memcpy(serial_number, usbcmd->serial_number, required_length - 1);
        result_b = ZSA_BUFFER_RESULT_SUCCEEDED;
    }
    return result_b;
}
 ...
 *
 */
static zsa_result_t usb_cmd_io(usbcmd_t usbcmd_handle,
                               uint32_t cmd,
                               void *p_cmd_data,
                               ...
                               uint32_t *cmd_status)
{
    RETURN_VALUE_IF_HANDLE_INVALID(ZSA_RESULT_FAILED, usbcmd_t, usbcmd_handle);
    ...
    result = ZSA_RESULT_FROM_BOOL((usbcmd = usbcmd_t_get_context(usbcmd_handle)) != NULL);

    if (ZSA_SUCCEEDED(result))
    {
        result = ZSA_RESULT_FROM_BOOL(cmd_data_size <= sizeof(usb_cmd_pkt.data));
    }

    if (ZSA_SUCCEEDED(result))
    {
        result = ZSA_RESULT_FROM_BOOL(payload_size <= INT32_MAX);
    }
 ...
 *
 */
zsa_result_t usb_cmd_read(usbcmd_t usbcmd_handle,
                          uint32_t cmd,
                          uint8_t *p_cmd_data,
                          size_t cmd_data_size,
                          uint8_t *p_data,
 ...
 *
 */
zsa_result_t usb_cmd_read_with_status(usbcmd_t usbcmd_handle,
                                      uint32_t cmd,
                                      uint8_t *p_cmd_data,
                                      size_t cmd_data_size,
                                      uint8_t *p_data,
 ...
 *
 */
zsa_result_t usb_cmd_write(usbcmd_t usbcmd_handle,
                           uint32_t cmd,
                           uint8_t *p_cmd_data,
                           size_t cmd_data_size,
                           uint8_t *p_data,
 ...
 *
 */
zsa_result_t usb_cmd_write_with_status(usbcmd_t usbcmd_handle,
                                       uint32_t cmd,
                                       uint8_t *p_cmd_data,
                                       size_t cmd_data_size,
                                       uint8_t *p_data,
 ...
 *
 */
zsa_result_t usb_cmd_stream_register_cb(usbcmd_t usbcmd_handle, usb_cmd_stream_cb_t *capture_ready_cb, void *context)
{
    zsa_result_t result;
    usbcmd_context_t *usbcmd;
 ...
 *
 */
zsa_result_t usb_cmd_get_device_count(uint32_t *p_device_count)
{
    zsa_result_t result = ZSA_RESULT_SUCCEEDED;
    struct libusb_device_descriptor desc;
    ...
    *p_device_count = 0;
    ...
    for (int loop = 0; loop < count; loop++)
    {
        ...
        if (desc.idVendor == ZSA_MSFT_VID)
        {
            if (desc.idProduct == ZSA_RGB_PID)
            {
                color_device_count += 1;
            }
            else if (desc.idProduct == ZSA_DEPTH_PID)
            {
 ...
 *
 */
zsa_result_t usb_cmd_path_get(usbcmd_t usbcmd_handle, uint8_t *p_bus, uint8_t *p_path, uint8_t path_size)
{
    zsa_result_t result = ZSA_RESULT_SUCCEEDED;
