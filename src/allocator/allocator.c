...
typedef struct
{
    ...
    // while holding lock
    zsa_memory_allocate_cb_t *alloc;
    zsa_memory_destroy_cb_t *free;
} allocator_global_t;
...
ZSA_DECLARE_GLOBAL(allocator_global_t, allocator_global_init);

//
// Simple counts of memory allocations for the purpose of detecting leaks of the ZSA SDK's larger memory objects.
//
...
// away from the user such that they can hold onto memory after zsa_device_close has been called, which would ultimately
...
// the entire process, not just the current session (zsa_device_open). As a result we must also count the active
// sessions so we can properly report leaks when all active sessions end (zsa_device_close).
...
// Count the number of active sessions for this process. A session maps to zsa_device_open
...
typedef struct _capture_context_t
{
    ...
    zsa_image_t image[IMAGE_TYPE_COUNT];

    float temperature_c; /** Temperature in Celsius */
} capture_context_t;

ZSA_DECLARE_CONTEXT(zsa_capture_t, capture_context_t);
...
zsa_result_t allocator_set_allocator(zsa_memory_allocate_cb_t allocate, zsa_memory_destroy_cb_t free)
{
    RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, allocate == NULL && free != NULL);
    RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, allocate != NULL && free == NULL);
...
long allocator_test_for_leaks(void)
{
    ...
    {
        logger_log(ZSA_LOG_LEVEL_CRITICAL,
                   __FILE__,
                   __LINE__,
...
{
    RETURN_VALUE_IF_HANDLE_INVALID(VOID_VALUE, zsa_capture_t, capture_handle);
    capture_context_t *capture = zsa_capture_t_get_context(capture_handle);
    long new_count;
    ...
    if (new_count == 0)
    {
        ...
        zsa_capture_t_destroy(capture_handle);
    }
}
...
{
    RETURN_VALUE_IF_ARG(ZSA_RESULT_FAILED, capture_handle == NULL);

    capture_context_t *capture = zsa_capture_t_create(capture_handle);
    zsa_result_t result = ZSA_RESULT_FROM_BOOL(capture != NULL);
...
{
    RETURN_VALUE_IF_HANDLE_INVALID(NULL, zsa_capture_t, capture_handle);

    capture_context_t *capture = zsa_capture_t_get_context(capture_handle);

    rwlock_acquire_read(&capture->lock);
    zsa_image_t *image = &capture->image[IMAGE_TYPE_COLOR];
    if (*image)
    {
        image_inc_ref(*image);
    }
...
zsa_image_t capture_get_depth_image(zsa_capture_t capture_handle)
{
    ...
    rwlock_acquire_read(&capture->lock);
    zsa_image_t *image = &capture->image[IMAGE_TYPE_DEPTH];
    if (*image)
    {
        image_inc_ref(*image);
    }
...
zsa_image_t capture_get_ir_image(zsa_capture_t capture_handle)
{
    ...
    rwlock_acquire_read(&capture->lock);
    zsa_image_t *image = &capture->image[IMAGE_TYPE_IR];
    if (*image)
    {
        image_inc_ref(*image);
    }
...
{
    RETURN_VALUE_IF_HANDLE_INVALID(NAN, zsa_capture_t, capture_handle);
