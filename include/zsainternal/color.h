 ...
 */
typedef void(color_cb_streaming_capture_t)(zsa_result_t result, zsa_capture_t capture_handle, void *callback_context);
 ...
 * container id associated with the device being using by this instance of zsa_device_open
 *
 * \param serial_number [IN]
 * serial number associated with the device being using by this instance of zsa_device_open
 *
 ...
 *
 * \return ::ZSA_RESULT_SUCCESS if successful. ::ZSA_RESULT_FAILED if an error occurs. If this API's fails then there is
 ...
 *
 * \return ::ZSA_RESULT_FAILED if the value could not be read. ::ZSA_RESULT_SUCCEEDED if successful. Details of the
 * error can be read from the debug output
 */
zsa_result_t color_get_control_capabilities(const color_t color_handle,
 ...
 * error can be read from the debug output
 */
zsa_result_t color_get_control(const color_t color_handle,
                               const zsa_color_control_command_t command,
 ...
 *
 * \return ::ZSA_RESULT_FAILED if the value could not be set. ::ZSA_RESULT_SUCCEEDED if successful. Details of the
 * error can be read from the debug output
 */
zsa_result_t color_set_control(const color_t color_handle,
                               const zsa_color_control_command_t command,
