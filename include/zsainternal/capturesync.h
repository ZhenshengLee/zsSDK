 ...
 */
ZSA_DECLARE_HANDLE(capturesync_t);

/** Creates an capturesync instance
 *
 * \param capturesync_handle
 * pointer to a handle location to store the handle. This is only written on ZSA_RESULT_SUCCEEDED;
 *
 * To cleanup this resource call capturesync_destroy().
 *
 * \ref ZSA_RESULT_SUCCEEDED is returned on success
 */
 ...
 * This function should not be called until all outstanding ::zsa_capture_t objects are freed.
 *
 ...
 *
 * \remarks
 * If ::ZSA_SUCCEEDED(result) is true then capture_raw must be valid. If ::ZSA_SUCCEEDED(result) is false then
