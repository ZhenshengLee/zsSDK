 ...
 */
#define QUEUE_DEFAULT_SIZE QUEUE_CALC_DEPTH(zsa_convert_fps_to_uint(ZSA_FRAMES_PER_SECOND_30), QUEUE_DEFAULT_DEPTH_USEC)
 ...
 *
 * \return ZSA_RESULT_SUCCEEDED if the device was opened, otherwise ZSA_RESULT_FAILED
 *
...
/** Adds a \ref zsa_capture_t object to the queue.
 *
 * \param queue_handle [in]
 *  A queue handle
 *
 ...
 */
void queue_push(queue_t queue_handle, zsa_capture_t capture_handle);

/** Adds a \ref zsa_capture_t object to the queue. If the queue is full prior to adding capture_handle, then the capture
 ...
 */
void queue_push_w_dropped(queue_t queue_handle, zsa_capture_t capture_handle, zsa_capture_t *dropped_handle);

/** Removes a \ref zsa_capture_t object from the queue.
 *
 * \param queue_handle [in]
 *  A queue handle
 *
 ...
 * arrived then NULL is returned. A timeout of ZSA_WAIT_INFINITE will wait indefinitely.
 *
 * \param capture_handle [out]
 * The location to write the memory object to that was just popped off the stack
 *
 * returns \ref ZSA_WAIT_RESULT_SUCCEEDED if successful, \ref ZSA_WAIT_RESULT_TIMEOUT if no data was available in the
 * period specified, \ref ZSA_WAIT_RESULT_FAILED if the queue was stopped/destroyed while waiting.
 */
zsa_wait_result_t queue_pop(queue_t queue_handle, int32_t wait_in_ms, zsa_capture_t *capture_handle);
