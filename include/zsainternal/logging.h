...
extern char ZSA_ENV_VAR_LOG_TO_A_FILE[];

#define ZSA_ENABLE_LOG_TO_A_FILE "ZSA_ENABLE_LOG_TO_A_FILE"
#define ZSA_RECORD_ENABLE_LOG_TO_A_FILE "ZSA_RECORD_ENABLE_LOG_TO_A_FILE"
 ...
 *
 * \remarks
 * See \ref zsa_set_debug_message_handler for more detailed documentation.
 */
zsa_result_t logger_register_message_callback(zsa_logging_message_cb_t *message_cb,
                                              void *message_cb_context,
                                              zsa_log_level_t min_message_level);

void logger_log(zsa_log_level_t level, const char *file, const int line, const char *format, ...);

FORCEINLINE zsa_result_t
TraceError(zsa_result_t result, const char *szCall, const char *szFile, int line, const char *szFunction)
{
    if (ZSA_FAILED(result))
    {
        logger_log(ZSA_LOG_LEVEL_ERROR, szFile, line, "%s returned failure in %s()", szCall, szFunction);
    }
    return result;
}
FORCEINLINE zsa_buffer_result_t
TraceBufferError(zsa_buffer_result_t result, const char *szCall, const char *szFile, int line, const char *szFunction)
{
    if (result == ZSA_BUFFER_RESULT_FAILED)
    {
        logger_log(ZSA_LOG_LEVEL_ERROR, szFile, line, "%s returned failure in %s()", szCall, szFunction);
    }
    return result;
}
FORCEINLINE zsa_wait_result_t
TraceWaitError(zsa_wait_result_t result, const char *szCall, const char *szFile, int line, const char *szFunction)
{
...
FORCEINLINE zsa_result_t TraceReturn(zsa_result_t result, const char *szFile, int line, const char *szFunction)
{
    if (ZSA_FAILED(result))
    {
        logger_log(ZSA_LOG_LEVEL_ERROR, szFile, line, "%s() returned failure.", szFunction);
    }
    else
    {
        logger_log(ZSA_LOG_LEVEL_TRACE, szFile, line, "%s() returned success.", szFunction);
    }
    return result;
}
...
FORCEINLINE void TraceArg(int result, const char *szFile, int line, const char *szFunction, const char *szExpression)
{
    if (!result)
    {
        logger_log(ZSA_LOG_LEVEL_ERROR, szFile, line, "Invalid argument to %s(). %s", szFunction, szExpression);
    }
}
                                    ...
                                    void *pHandleValue)
{
    if (!result)
    {
        logger_log(ZSA_LOG_LEVEL_ERROR,
                   szFile,
                   line,
...
/*
    ...
    Calling other functions:
        If the callee function returns a zsa_result_t, wrap the call in TRACE_CALL.
            TRACE_CALL will pass through the result, and trace the error information if one occurs
        If the callee function returns another type, ZSA_RESULT_FROM_BOOL will convert a boolean success expression to a
            a zsa_result_t, and trace the error information if one occurs
    ...
    Checking for failures:
        The preferred pattern is to initialize a result variable as ZSA_RESULT_SUCCEEDED, then update that variable with
        each call or operation in the function. If a subsequent operation shouldn't be performed after a failure,
        wrap it in an "if (ZSA_SUCCEEDED(result))" block

    Cleaning up from errors:
        Generally, if a function returns ZSA_RESULT_FAILED, it should not have any side effects on persistent state.
   This means cleaning up any memory that was allocated before the failure, or undo-ing any operations that were
   partially completed.

        Functions should check for failure with a ZSA_FAILED(result) check, and perform all the needed cleanup. If there
...
// Trace the output of a call to another function that returns a zsa_result_t
...
// Convert a bool success epxression to a zsa_result_t. Trace the result of the conversion on failures.
#define ZSA_RESULT_FROM_BOOL(_call_)                                                                                   \
    TraceError((_call_) ? ZSA_RESULT_SUCCEEDED : ZSA_RESULT_FAILED, #_call_, __FILE__, __LINE__, __func__)
...
#define RETURN_VALUE_IF_ARG(_fail_value_, _expression_)                                                                \
    ...
    {                                                                                                                  \
        ...
        TraceReturn(ZSA_RESULT_FAILED, __FILE__, __LINE__, __func__);                                                  \
        return _fail_value_;                                                                                           \
    }                                                                                                                  \
    else                                                                                                               \
    {                                                                                                                  \
...
#define LOG_TRACE(message, ...)                                                                                        \
    logger_log(ZSA_LOG_LEVEL_TRACE, __FILE__, __LINE__, "%s(). " message, __func__, __VA_ARGS__)
#define LOG_INFO(message, ...)                                                                                         \
    logger_log(ZSA_LOG_LEVEL_INFO, __FILE__, __LINE__, "%s(). " message, __func__, __VA_ARGS__)
#define LOG_WARNING(message, ...)                                                                                      \
    logger_log(ZSA_LOG_LEVEL_WARNING, __FILE__, __LINE__, "%s(). " message, __func__, __VA_ARGS__)
#define LOG_ERROR(message, ...)                                                                                        \
    logger_log(ZSA_LOG_LEVEL_ERROR, __FILE__, __LINE__, "%s(). " message, __func__, __VA_ARGS__)
#define LOG_CRITICAL(message, ...)                                                                                     \
    logger_log(ZSA_LOG_LEVEL_CRITICAL, __FILE__, __LINE__, "%s(). " message, __func__, __VA_ARGS__)
